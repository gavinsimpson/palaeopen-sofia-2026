...
# Multivariate data {background-color="black" background-gradient="linear-gradient(to bottom, #283b95, #17b2c3)"}
...
# Topic models {background-color="black" background-gradient="linear-gradient(to bottom, #283b95, #17b2c3)"}
...
# Rate of change {background-color="black"background-gradient="linear-gradient(to bottom, #283b95, #17b2c3)"}
...
## aber ages
aber_age <- read_rds(here("00-data", "abernethy-sample-age.rds"))
## or:
...
if (FALSE) { ## Don't refit in the slides! Needs cmdstanr backend - memory!
    ...
    m  <- read_rds(here("04-fancy-stuff/models/aber-dirichlet.rds"))
    m2 <- read_rds(here("04-fancy-stuff/models/aber-dirichlet-phi.rds"))
}
...
if (FALSE) { # something weird going on if you process models locally, fitted elsewhere
    ...
    fv1 <- read_rds(here("04-fancy-stuff/models/aber-dirichlet-fitted-summary.rds"))
    fv2 <- read_rds(here("04-fancy-stuff/models/aber-dirichlet-phi-fitted-summary.rds"))
    fv1_samps <- read_rds(here("04-fancy-stuff/models/aber-dirichlet-fitted-samples.rds"))
    fv2_samps <- read_rds(here("04-fancy-stuff/models/aber-dirichlet-phi-fitted-samples.rds"))
}
...
if (FALSE) {
    ...
    fd1_samps <- read_rds(here("04-fancy-stuff/models/aber-dirichlet-fitted-samples-derivs.rds"))
    fd2_samps <- read_rds(here("04-fancy-stuff/models/aber-dirichlet-phi-fitted-samples-derivs.rds"))
}
```
...
::: {.column width=50%}
![](/resources/page1-1280px-Dirichlet.jpg)
:::
...
```{r abernethy-strat-plot, out.width = "90%"}
#| echo: false
aber_tmp <- aber |>
    bind_cols(aber_age) |>
    as.data.frame()
...
```{r plot-dirichlet-trends, dependson = -3}
#| echo: false
fvs |>
    ggplot(aes(x = age, y = proportion, colour = association)) +
...
```{r plot-dirichlet-derivatives, dependson = -4, cache=TRUE}
#| echo: false
